...
def upload_dataframe(client,dataset_id,table_id,df,truncate):
    ...
    if 'update_time' not in df.columns:
        df['update_time'] = datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%dT%H:%M:%S')
    print(df)
    print(table_id)
...
def scheduled_query(transfer_client,dataset_id,table_id,query_string,schedule):
...
def create_view(client,dataset_id,view_id,query_string):
    ...
    table_id = f"{client.project}.{dataset_id}.{view_id}"
